    ...
    private async Task SaveOrderToDatabase(StockOrder order)
    {
        try
        {
            ...
            var trader = await dbContext.Traders
                .Include(t => t.Orders) // Include Orders for validation/logging
            ...
            // Check and update account balance if it's a "buy" order
            ...
            // Add the order to the trader's collection
            trader.Orders.Add(order);
            Console.WriteLine("[DEBUG] Order added to trader's collection.");
