...
public class RabbitMqConsumer
{
    ...
    public async Task StartConsumingAsync(string queueName, Func<string, Task> onMessageReceived)
    {
        ...
        consumer.ReceivedAsync += async (model, ea) =>
        {
            ...
            if (stockOrder != null)
            {
                Console.WriteLine($"[x] Processing order: TraderId: {stockOrder.TraderId }, Stock: {stockOrder.StockSymbol}, Quantity: {stockOrder.Quantity}, Price: {stockOrder.Price}, Type: {stockOrder.OrderType}");
