...
public class RabbitMqConsumer
{
    ...
    public async Task SaveOrderToDatabase(StockOrder order)
    {
        try
        {
            ...
            Console.WriteLine($"[DEBUG] Trader found: {trader.Name}, Account Balance: {trader.AccountBalance}");
            ...
            if (order.OrderType.ToLower() == "buy")
            {
                ...
                Console.WriteLine($"[DEBUG] Deducted {totalCost} from TraderId: {order.TraderId}. New balance: {trader.AccountBalance}");

                // Log state before marking as modified
                Console.WriteLine($"[DEBUG] Trader state before marking modified: {dbContext.Entry(trader).State}");

                dbContext.Entry(trader).State = EntityState.Modified;

                // Log state after marking as modified
                Console.WriteLine($"[DEBUG] Trader state after marking modified: {dbContext.Entry(trader).State}");
            }
