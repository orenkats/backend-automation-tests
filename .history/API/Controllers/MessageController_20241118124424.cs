...
public class MessageController : ControllerBase
{
    ...
    {
        if (string.IsNullOrEmpty(request.QueueName) || request.Order == null || string.IsNullOrEmpty(request.Order.StockSymbol))
            return BadRequest("Queue name and order details are required.");

        try
        {
            Console.WriteLine($"SendMessage called for queue: {request.QueueName}, StockSymbol: {request.Order.StockSymbol}, Quantity: {request.Order.Quantity}");
